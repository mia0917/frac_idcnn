...
if opt.mode == 'train':
    ...
    opt.checkpoint = 'out_' + 'train' + '_' + opt.opt_loss + '_' + str(opt.epoch) + '_epoch_' + str(
        opt.tv_lambda) + '_lambda'
    if opt.opt_loss == 'frac_tv':
...
def deconv(batch_input, out_channels):
    with tf.variable_scope("deconv"):
        ...
        #     => [batch, out_height, out_width, out_channels]
        conv = tf.nn.conv2d_transpose(batch_input, filter, [batch, in_height, in_width, out_channels], [1, 1, 1, 1],
                                      padding="SAME")
        return conv
...
def batchnorm(input):
    with tf.variable_scope("batchnorm"):
        ...
        scale = tf.get_variable("scale", [channels], dtype=tf.float32,
                                initializer=tf.random_normal_initializer(1.0, 0.02, seed=0))
...
def create_model(inputs, targets):
    ...
    if opt.opt_loss == 'only_d':
        ...
        loss = tf.reduce_mean(tf.square(targets - outputs)) + lambda_tv * tf.reduce_mean(
            tf.image.total_variation(outputs)) / (SCALE_SIZE * SCALE_SIZE)
    elif opt.opt_loss == 'frac_tv':
        loss = tf.reduce_mean(tf.square(targets - outputs)) + lambda_tv * tf.reduce_mean(
            frac_total_variation(outputs, opt.v)) / (SCALE_SIZE * SCALE_SIZE)
...
def append_index(filesets, step=False):
    ...
    if os.path.exists(index_path):
        ...
        if step:
            index.write("<th>step</th>")
        index.write(
            "<th>name</th><th>input</th><th>output</th><th>target</th><th>PSNR</th><th>SSIM</th><th>UQI</th><th>DG</th></tr>")
...
def frac_total_variation(images, v=0.5, name=None):
    ...
    if ndims == 4:
        ...
        a4l = g(n - v - 1) * (1 - v * v / 4) / (math.factorial(n - 1) * g(-v)) + g(n - v - 2) * (-v / 4 + v * v / 8) / (
        math.factorial(n - 2) * g(-v))
        a3l = frac_op_creater(3)
        a2l = frac_op_creater(2)
        a1l = frac_op_creater(1)
        a0 = 1 - v * v / 2 - v * v * v / 8
