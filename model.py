...
parser.add_argument("--tv_lambda", type=float)
...
opt.input_dir = 'small_data_' + opt.mode
...
def load_examples():
    ...
    with tf.name_scope("load_images"):
        path_queue = tf.train.string_input_producer(input_paths, shuffle=True, seed=0)
...
def conv(batch_input, out_channels, stride):
    with tf.variable_scope("conv"):
        ...
        filter = tf.get_variable("filter", [3, 3, in_channels, out_channels], dtype=tf.float32, initializer=tf.random_normal_initializer(0, 0.02, seed=0))
...
def deconv(batch_input, out_channels):
    with tf.variable_scope("deconv"):
        ...
        filter = tf.get_variable("filter", [3, 3, out_channels, in_channels], dtype=tf.float32, initializer=tf.random_normal_initializer(0, 0.02, seed=0))
...
def batchnorm(input):
    with tf.variable_scope("batchnorm"):
        ...
        scale = tf.get_variable("scale", [channels], dtype=tf.float32, initializer=tf.random_normal_initializer(1.0, 0.02, seed=0))
...
def frac_total_variation(images, v = 0.5, name=None):
    ...
    if ndims == 4:
      ...
      # 简化堆公式
        ...
        a5l = g(n-v-1)*(-v/4+v*v/8) / (math.factorial(n-1)*g(-v))
        a4l = g(n-v-1)*(1-v*v/4) / (math.factorial(n-1)*g(-v)) + g(n-v-2)*(-v/4+v*v/8) / (math.factorial(n-2)*g(-v))
        ...
        filters = [t_filter, rt_filter, r_filter, rb_filter, b_filter, lb_filter, l_filter, lt_filter]
        # generate the fractional filter

        res = 0

        for filter in filters:
            filter = tf.constant(filter, shape=(7, 7, 1, 1), dtype=tf.float32)
            filter = tf.stop_gradient(filter)
            mat = tf.nn.conv2d(images, filter, [1, 1, 1, 1], 'SAME', True)

            res += math_ops.reduce_sum(math_ops.abs(mat), axis=[1, 2, 3])
...
# def total_variation_2norm(images, name=None):
#
...
#         sum_axis = [1, 2, 3]
#
#         tot_var = math_ops.reduce_sum(math_ops.sqrt(math_ops.square(pixel_dif1)+math_ops.square(pixel_dif2)), axis=sum_axis)
#
#     return tot_var
...
with sv.managed_session() as sess:
    ...
    if opt.checkpoint is not None:
        print("get model from", opt.checkpoint)
